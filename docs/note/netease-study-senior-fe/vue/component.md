---
lang: zh-CN
description: Vue.js 注册（全局注册、局部注册、全局导入、按需引入）、生命周期、动态组件（<component>、<keep-alive>）、八皇后问题选择功能。
meta:
  - name: keywords
...
Vue.js 注册（全局注册、局部注册、全局导入、按需引入）、生命周期、动态组件（`<component>`、`<keep-alive>`）、八皇后问题选择功能。
